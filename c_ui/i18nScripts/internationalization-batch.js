...
import { writeJsonFile, getFileJson } from "./utils.js"; // 注意这里要改为 .js
...
async function processFile(filePath) {
  ...
  const type = filePath.includes(".ts") ? "ts" : "vue";
  if (type === "vue") {
    // ================ 处理HTML部分 ================
      ...
      text = text.replace(
        ...
        (match, quote, text) => {
          if (match.includes(":'") || match.includes(": '")) {
            ...
            return `${type === "ts" ? "i18n.global.t" : "$t"}('${key}')`;
          }
          return match;
        }
      );
  ...
  content = content.replace(
    ...
    (match, quote, text) => {
      ...
      langMap.set(key, text);
      if (text.includes("<br/>")) {
        return `${type === "ts" ? "i18n.global.t" : "$t"}('${key.replace(
          "<br/>",
          ""
        )}') + <br/>`;
      }
  ...
  content = content.replace(
    ...
    (match, text) => {
      ...
      return (
        "`${" +
        `${type === "ts" ? "i18n.global.t" : "$t"}('${key}')}${isInclude ? "_" : ""
        }$`
      );
    }
  );
  ...
  content = content.replace(
    ...
    (match, text) => {
      ...
      langMap.set(key, text);
      return (
        "`${" + `${type === "ts" ? "i18n.global.t" : "$t"}('${key}')}` + ".xlsx`"
      );
    }
  );
  //  6. `xxx`
...
(async () => {
  ...
  const jsFiles = glob.sync(`../src/**/*.ts`);
...
// src\views\assembly\testField\dhxGanttChart\components\ganttChart.vue