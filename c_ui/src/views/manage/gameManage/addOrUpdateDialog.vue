...
<template>
  <c-dialog v-model="open" :title="title" width="700" :modal="true">
    <el-form :model="form" label-width="70" ref="formRef" :rules="rules">
      <el-form-item :label="$t('游戏名')" prop="name">
      ...
      <el-form-item :label="$t('分类')" prop="type">
        <el-radio-group v-model="form.type" class="ml-4">
          <el-radio :label="'0'">{{ $t('单机') }}</el-radio>
          <el-radio :label="'1'">{{ $t('手游') }}</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item :label="$t('官网')" prop="url">
        <el-input v-model="form.url" clearable />
      </el-form-item>
      <el-form-item :label="$t('评分')" prop="rate">
...
const rules = ref({
  ...
  name: [{ required: true, message: $t('请输入游戏名称'), trigger: 'blur' }],
  type: [{ required: true, message: $t('请选择分类'), trigger: 'blur' }],
  rate: [{ required: true, message: $t('请输入评分'), trigger: 'blur' }]
});
